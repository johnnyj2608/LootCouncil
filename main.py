...
def main():
    ...
    for key in items:
        ...
        temp = [[x for x in sub_list if x] for sub_list in temp]
    ...
    report_names = set(wcl.get_names(code='bPpcTmQrzGXdMxA6'))
    delete_keys = []
    for key in raiders:
        if key in report_names:
            raiders[key].update(wcl.get_spec(name=key))
            if raiders[key]['Spec'] == 'Shadow':    # Shadow default queries are hps for some reason
